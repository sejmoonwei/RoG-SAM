...
def display_image_with_point(image, point= [0,0], box = [0,0,0,0]):
    ...
    plt.axis("off")
    plt.savefig('/data1/samgrasp/dataset/test/Cornell_test/show2/rgb.png', bbox_inches='tight', pad_inches=0)
    # plt.show()
    plt.close()  # 关闭图形以释放内存
...
def display_image_with_grasp(image, grasp):
    ...
    plt.axis("off")
    plt.savefig('/data1/samgrasp/dataset/test/Cornell_test/show2/rgbgrasp.png', bbox_inches='tight', pad_inches=0)
    plt.close()  # 关闭图形以释放内存
...
def display_labels_with_point(data, point= [0,0], box = [0,0,0,0]):
    ...
    plt.tight_layout()
    plt.savefig('/data1/samgrasp/dataset/test/Cornell_test/show2/heatmap_label.png', bbox_inches='tight', pad_inches=0)
    # plt.show()
    plt.close()  # 关闭图形以释放内存
...
def show_heatmap(able_pred):
    ...
    # 可视化为热度图
    plt.imshow(able_pred_np, cmap='jet', interpolation='nearest',vmin=0,vmax=0.8)
    # plt.colorbar()  # 添加颜色条
    ...
    plt.savefig('/data1/samgrasp/dataset/test/Cornell_test/show2/heatmap.png', bbox_inches='tight', pad_inches=0)
    plt.close()  # 关闭图形以释放内存
...
args = cfg.parse_args()
#--------------------args--------------------------#
args.pretrain    = '/data1/ori/checkpoint/cornell_994.pth'
args.image_size  = 512
args.mask_size   = 256
args.out_size    = 512
args.file        = '/data1/samgrasp/dataset/cornell_adapt/All/pcd0148r.png'
#pcd0325r.png 0108  514
args.label_path  = args.file.replace('r.png','grasp.mat')
#--------------------------------------------------#
