...
if __name__ == '__main__':
    ...
    for i in range(len(labels.grasps)):
        ...
        with torch.no_grad():
            ...
            show_heatmap(combined_slices[0].cpu()) #heatmap
            display_image_with_point1(img_in.squeeze(0).permute(1,2,0).cpu().numpy(),pt_coord,box_cup)  # 320,320,3   255 RGB+prompt
            display_image_with_grasp(img_in.squeeze(0).permute(1,2,0).cpu().numpy(),grasp) #grasp
            # display_image_with_hm(img_in.squeeze(0).permute(1,2,0).cpu().numpy(),pt_coord,box_cup,ang.cpu())

            lbl = torch.from_numpy(labels.grasps[i][:1]).unsqueeze(0)
