...
def gen_mat(label_file):
    scale = 0.5 #1024 to 512
    label_mat = np.zeros((4, 512, 512), dtype=np.float64) #pos cls angle width
    with open(label_file) as f:
        labels = f.readlines()
        for label in labels:
            label = label.strip().split(';')
            row = int(float(label[1]) * scale)
            col = int(float(label[0]) * scale)
            ...
            # 设置抓取角
            if len(label) != 5:  #
                raise ValueError('grasp label error')
            else:
            ...
            label_mat[3,startx:endx,starty:endy] = float(label[-2]) * scale  # 设置抓取宽度
    # 0:position 1:label 2:angle 3:w
    #---------------vis------------------#
    # png_file = label_file.replace('grasps.txt','RGB.png')
    ...
    #         x = int(x)
    #         y = int(y)
    #         w = float(label_mat[3][y,x]) / 2 # width
