...
class GraspMat:
    ...
    def __init__(self, file, ins_mask_path, mode):
        ...
        if mode == 'subset':
            ...
            while not found_valid_grasp:
                ...
                #-----------------------------------------------------------#
                positions2 = (instance_msk == 16) # 480,640 array
                expanded_position2 = np.expand_dims(positions2,axis=0) # 1,480,640
                expanded_position2 = np.repeat(expanded_position2, grasp.shape[0], axis=0) # 4,480,640
                grasp_ins2 = np.where(expanded_position2, grasp, 0) # 4,480,640
                #-----------------------------------------------------------#
        ...
        self.grasp2 = self.preprocess_image(grasp_ins2)           #4,512,512
    ...
    def crop(self, bbox):
        ...
        self.graso2 = self.grasp2[:, bbox[1]:bbox[3], bbox[0]:bbox[2]]
    ...
    def decode(self, angle_cls):
        ...
        self.grasp2 = self._decode(self.grasp2, angle_cls)
