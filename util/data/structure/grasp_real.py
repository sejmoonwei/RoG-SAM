...
def draw_grasp(label_mat,png_file):
    im = cv2.imread(png_file)
    mask = torch.from_numpy( label_mat[0]==1 )
    indices = torch.nonzero(mask, as_tuple=False)
    list_of_coord = [tuple(idx) for idx in indices ]
...
class GraspMat:
    ...
    def __init__(self, file, ins_mask_path, mode):
        ...
        # draw_grasp(self.grasp,'/data1/samgrasp/dataset/real1/pcd2000r.png')
        if mode == 'subset':
            ...
            while not found_valid_grasp:
                ...
                expanded_position = np.repeat(expanded_position, self.grasp.shape[0], axis=0) #4,480,640
                grasp_ins = np.where(expanded_position, self.grasp, 0) #4,480,640
