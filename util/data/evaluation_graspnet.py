...
def evaluation(able_out, angle_out, width_out, target, top_k, angle_k=120, eval_mode='peak', angle_th=30, iou_th=0.25, bottom=30, desc='1'):
    """
    评估预测结果
    ...
    # 搜索超过抓取置信度的待评估点
    threshold_abs = 0.2 # 0.3 [0.5 58%]  [
    if eval_mode == 'peak':
        min_distance = 20
        pred_pts = peak_local_max(able_out, num_peaks=top_k, min_distance=min_distance, threshold_abs=threshold_abs)
