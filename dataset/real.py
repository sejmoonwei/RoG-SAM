...
class REAL(Dataset):
    ...
    def _load_split(self):
        ...
        rgb_images = [file for file in all_files if file.endswith('.png') and '_mask' not in file]
        return rgb_images

    def _get_path(self, item):
        file_name = os.path.splitext(item)[0]
        base_name = file_name[:-1]
        img_path = os.path.join(self.data_path, item)
        anno_path = os.path.join(self.data_path, base_name + 'grasp.mat')
        ins_mask_path = os.path.join(self.data_path, file_name + '_mask.png')
    ...
    def __getitem__(self, item):
        ...
        if 'click' in self.prompt and click_mask_np.max() != 0:
            ...
            ########
            point_label, pt = random_click(click_mask, point_label)  # in: click_mask_np 512,512  0-1
            x = pt[1]
            y = pt[0]
            pt[0] = x
            pt[1] = y
...
if __name__ == '__main__':
    ...
    REAL_train_dataset = REAL(args,transform_train, data_path = '/data1/samgrasp/dataset/img4')
